    ...
    <div className="flex">
      ...
      >
        ...
        <div className="flex flex-col gap-2">
          <div
            className={`flex items-center p-4 rounded-lg bg-[#1e293b] relative ${
              isCollapsed ? 'justify-center' : ''
            }`}
          >
            <img src={db} alt="Dashboard" className="w-6 h-6 mr-3" />
            {!isCollapsed && (
              <Link to="/dashboard" className="font-inter text-sm leading-6 text-[#3b82f6]">
                Dashboard
              </Link>
            )}
          </div>
          ...
          <div
            className={`flex items-center p-4 rounded-lg hover:bg-[#1e293b] ${
              isCollapsed ? 'justify-center' : ''
            }`}
          >
            <img src={course} alt="Courses" className="w-6 h-6 mr-3" />
            {!isCollapsed && (
              <Link to="/back-course" className="font-inter text-sm leading-6">
                Courses
              </Link>
            )}
          </div>
          ...
          >
            ...
            {!isCollapsed && (
              <Link to="/communication" className="font-inter text-sm leading-6">
                Communication
              </Link>
            )}
          </div>
          ...
          >
            ...
            {!isCollapsed && (
              <Link to="/revenue" className="font-inter text-sm leading-6">
                Revenue
              </Link>
            )}
          </div>
          ...
          >
            ...
            {!isCollapsed && (
              <Link to="/setting" className="font-inter text-sm leading-6">
                Setting
              </Link>
            )}
          </div>
      ...
      <div className="flex-1 p-8">
        ...
        {/* The content of the page will be displayed here */}
        <div className="mt-4">{/* Page-specific content */}</div>
      </div>
    </div>
  );
};
