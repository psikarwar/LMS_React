  ...
  return (
    <div className="px-4 sm:px-6 py-6 max-w-screen-xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-6">
        <h2 className="font-inter font-semibold text-2xl sm:text-3xl leading-[140%] text-gray-900">
          Top Categories
        </h2>
        <button className="px-4 sm:px-6 py-2 rounded-lg bg-transparent border-none cursor-pointer text-blue-500 font-inter font-medium text-sm sm:text-base leading-[160%]">
          See All
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6 sm:gap-10">
        {categories.map((category, index) => (
          <div
            key={index}
            className="flex flex-col items-center p-6 bg-white rounded-xl border border-gray-200 text-center shadow-sm"
          >
            <div className="w-[70px] sm:w-[100px] h-[70px] sm:h-[100px] mx-auto mb-4 bg-blue-100 rounded-full flex items-center justify-center">
              <img src={category.icon} alt={category.name} className="w-8 sm:w-10 h-8 sm:h-10" />
            </div>
            <h3 className="font-inter font-semibold text-base sm:text-lg leading-[150%] text-gray-900 mb-2">
              {category.name}
            </h3>
            <p className="font-inter font-normal text-sm sm:text-base leading-[160%] text-gray-700">
              {category.courses} Courses
            </p>
          </div>
        ))}
